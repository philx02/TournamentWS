...
class WebSocketConnection : public ISender, public std::enable_shared_from_this< WebSocketConnection< PayloadHandler > >
{
public:
  WebSocketConnection(boost::asio::io_service &iIoService, const PayloadHandler &iPayloadHandler, std::size_t iBufferSize = 16*1024)
    : mSocket(iIoService)
  ...
  virtual void send(const std::string &iMessage)
  {
    ...
    //boost::asio::async_write(mSocket, boost::asio::buffer(wResponseHeader), boost::bind(&WebSocketConnection::handleWritePayloadHeader, shared_from_this(), boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
...
private:
  ...
  {
    boost::asio::async_read_until(mSocket, mPayloadBuffer, "\r\n", boost::bind(&WebSocketConnection::handleReadHandshake, this->shared_from_this(), boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
  }
  ...
  void handleReadHandshake(const boost::system::error_code &iError, std::size_t iBytesTransfered)
  {
    if (!iError)
    {
      ...
      if (wLine.empty())
      {
        ...
        mResponse.append("\r\n\r\n");
        boost::asio::async_write(mSocket, boost::asio::buffer(mResponse), boost::bind(&WebSocketConnection::handleWriteHandshake, this->shared_from_this(), boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
      }
      else
      {
  ...
  void listenForPayload(std::size_t iNumberOfBytes)
  {
    ...
    boost::asio::async_read(mSocket, mPayloadBuffer, wCompletionCondition, boost::bind(&WebSocketConnection::handleFragment, this->shared_from_this(), boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
  }
  ...
  void handleWritePayloadHeader(const boost::system::error_code &iError, std::size_t iBytesTransfered)
  {
    if (!iError)
    {
      boost::asio::async_write(mSocket, boost::asio::buffer(mResponse), boost::bind(&WebSocketConnection::handleWritePayload, this->shared_from_this(), boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
    }
  }
